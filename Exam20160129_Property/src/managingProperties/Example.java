...
public class Example {
	...
	/**
	 * This code serves the only purpose of checking whether your entered the
	 * required personal information in file Student.txt
	 */
	...
	static {
		...
		try {
			...
			try {
				p.store(new FileOutputStream(STUDENT_TXT),
						"Enter your identification information (Note: ID is Matricola)");
			} catch (IOException e1) {
				JOptionPane.showMessageDialog(null,
						"The current project folder is not accessible, please notify your teacher!", "Error",
		...
		if (p.getProperty("ID", "").equals("")) {
			...
			System.err.println("Project by " + p.getProperty("FirstName") + " " + p.getProperty("LastName") + " ("
					+ p.getProperty("ID") + ")\n");
			System.err.flush();
		}
	}
