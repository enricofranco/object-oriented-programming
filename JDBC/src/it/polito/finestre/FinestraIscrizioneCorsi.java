...
public class FinestraIscrizioneCorsi extends JFrame {
	...
	public FinestraIscrizioneCorsi(final DB db) {
		...
		setResizable(false);
		// setto il dispose automatico della finestra se il l'icona X viene
		// premuta
		...
		/*
		 * Listen anonimo sul Bottone Iscrivi. Alla pressione ottiene il codice
		 * cliente e il codice corso e aggiunge l'iscrizione relativa nel DB. Se
		 * viene eseguita inserisce nel JTextField un messaggio di successo,
		 * altrimenti di insuccesso.
		 * 
		 * Inoltre aggiorna gli elementi presenti nella tendina del codice
		 * cliente e del codice corso in modo tale da rispettare eventuali
		 * modifiche che vengono effettuate nel DB dalla funzione di
		 * aggiungiIscrizione o da fattori esterni.
		 * 
		 */
		iscrivi.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent e) {
				try {
					...
					boolean ok = db.aggiungiIscrizione(Long.parseLong("" + s1[0]),
							Long.parseLong("" + cod_cliente.getText()));
					if (ok == true)
