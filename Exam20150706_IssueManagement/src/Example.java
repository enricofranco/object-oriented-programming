...
public class Example {
	...
	public static void main(String[] args) throws TicketException {
		...
		ts.openTicket("alpha", "/System/SubB", "The colors in the diagram are hard to tell apart",
		...
		System.out.println(
				"User " + t.getAuthor() + " created ticket " + t.getId() + " on component " + t.getComponent());
	...
	static {
		...
		if (p.getProperty("ID", "").equals("")) {
			...
			System.out.println("Project by " + p.getProperty("FirstName") + " " + p.getProperty("LastName") + " ("
					+ p.getProperty("ID") + ")");
			System.out.flush();
		}
	}
